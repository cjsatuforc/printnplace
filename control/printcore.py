...
def setup_logging(out=stderr, filepath = None, reset_handlers = False):
...
class printcore():
    ...
    def connect(self, port = None, baud = None, dtr=None):
        ...
        """
	logging.info("connecting to %s, %d"%(port,baud))
        if self.printer:
            self.disconnect()
        if port is not None:
            self.port = port
    ...
    def _readline(self):
        try:
            ...
            if 'Bad file descriptor' in e.args[1]:
                self.logError((u"Can't read from printer (disconnected?) (SelectError {0}): {1}").format(e.errno, decode_utf8(e.strerror)))
                return None
            else:
                self.logError((u"SelectError ({0}): {1}").format(e.errno, decode_utf8(e.strerror)))
                raise
        except SerialException as e:
            self.logError((u"Can't read from printer (disconnected?) (SerialException): {0}").format(decode_utf8(str(e))))
            return None
        except socket.error as e:
            self.logError((u"Can't read from printer (disconnected?) (Socket error {0}): {1}").format(e.errno, decode_utf8(e.strerror)))
            return None
        except OSError as e:
            if e.errno == errno.EAGAIN:  # Not a real error, no data was available
                return ""
            self.logError((u"Can't read from printer (disconnected?) (OS Error {0}): {1}").format(e.errno, e.strerror))
            return None
...
#            return
        ...
        if self.printing and self.queueindex < len(self.mainqueue):
            line = self.mainqueue.idxs(self.queueindex)
            tline = line
